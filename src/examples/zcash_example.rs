...
//! This module implements a zcash style private transaction protocol,
//! where one may transfer arbitrary amount of tokens through a private
//! transaction.
//!
...
//!     - an UTXO proof proves that input notes are valid w.r.t. the given
//!       merkle root
...
impl TypeACircuit for ZcashPredicateCircuit {
    ...
    //
    // Our death circuit does nothing, so we don't need to pad it.
    const PAD_GATES: usize = 0;
}

// Extra, application dependent logics are defined in this circuit.
impl TypeBCircuit for ZcashPredicateCircuit {
    // In this example, death circuit only constraint
    // that it uses a same local data commitment as the
    // birth predicate
    fn gen_death_circuit_core(
        ...
        Ok(ZcashPredicateCircuit(PredicateCircuit(death_circuit)))
    }
}

impl<'a> PredicateOps<'a> for ZcashPredicate {
    ...
    > {
        ...
        let death_predicate_circuit =
            ZcashPredicateCircuit::preprocessed_death_circuit(entire_input_size)?;
        ...
        let mut birth_predicate_circuit =
            ZcashPredicateCircuit::preprocessed_birth_circuit(entire_input_size)?;
        ...
        let unmerged_inner_domain_size = birth_predicate_circuit.0 .0.eval_domain_size()?;

        let (dpc_pk, dpc_vk, (..)) = crate::proofs::transaction::preprocess(
            outer_srs,
            inner_srs,
            entire_input_size,
            unmerged_inner_domain_size,
        )?;
        Ok((
            dpc_pk,
            dpc_vk,
    ...
    fn finalize_for_proving(
        ...
        let mut final_circuit = if is_birth_predicate {
            ZcashPredicateCircuit::gen_birth_circuit(
                entire_input_notes,
            ...
            ZcashPredicateCircuit::gen_death_circuit(
                entire_input_notes,
                entire_output_records,
                memo,
                blinding_local_data,
...
mod test {
    ...
    fn test_zcash_example_transaction() -> Result<(), DPCApiError> {
        ...
        test_example_transaction_helper(
            &inner_srs,
            &outer_srs,
